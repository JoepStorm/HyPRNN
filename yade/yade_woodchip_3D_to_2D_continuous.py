...
parser.add_argument("--r1", type=float, default=0.875, help="nx/nz ratio ∈ (0, 1]")
parser.add_argument("--r2", type=float, default=0.875, help="ny/nz ratio ∈ [r1, 1]")
parser.add_argument("--nz", type=int, default=8, help="Largest dimension (number of spheres)")
...
spheres_per_clump = nz * max((nz * r1), 1) * max((nz * r2), 1)
if nz * r1 * r2 >= 2.7:
    num_chips = int(12000 / spheres_per_clump)
else:
    num_chips = int(8000 / spheres_per_clump)

# Relaxation options
relaxation_damping = 0.95
max_iter_gravity = None
max_iter_relaxing = 5000
