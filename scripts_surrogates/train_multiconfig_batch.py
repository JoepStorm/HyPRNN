"""
This file is used for training and evaluating many surrogate models on a dataset

There are 3 long lists of settings, corresponding to either D_all, D_geo, and D_mat.
Most settings are commented out, as training all models takes a very long time.

This is meant to be run on a cluster, where one job takes a single configuration, and trains that config many times to create a learning curve.
Note that for each configuration, for each value in 'training_samples_array', there are 'runs_per_setting' models trained.
In this default setup, this is 130 training runs per model configuration; which generally takes a few hours.
...
# ------------------------------- Dataset_mat (material property only) -------------------------------
# configurations = [
#     # --- NN configurations ---
#     # {
#     #     'name': 'nn_8_2',
